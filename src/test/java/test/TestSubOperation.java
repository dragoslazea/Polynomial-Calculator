...
// Clasa de test pentru operatia de scadere
public class TestSubOperation {
    private CalcModel model = new CalcModel();
    @ParameterizedTest
    @MethodSource("provideInput")
    void testSubtractionOperation(Polynomial p1, Polynomial p2, String expectedResult){
        model.setP(p1);
        model.setQ(p2);
    ...
    private static List<Arguments> provideInput(){
        ...
        argumentsList.add(Arguments.of(p1, p2, "X^3+X^2-X"));
        argumentsList.add(Arguments.of(p3, p4, "4X^5-6X^4+X^3-10X+2"));
        argumentsList.add(Arguments.of(p1, p3, "-4X^5+3X^4+2X^3+8X-1"));
        argumentsList.add(Arguments.of(p2, p4, "-3X^4+2X^3-X^2-X+1"));
        argumentsList.add(Arguments.of(p5, p1, "9X^7-2X^3"));
